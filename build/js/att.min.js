...
!function(){function t(){g=_.chain(_.range(v)).map(function(t){return{x:(1+t)*h/(v+1),y:14*x/16,alive:!0}}).value(),_.each(g,function(t,e){S.text(t.x,t.y,"").attr({id:"base"+e,"font-family":"FontAwesome","font-size":"40px",dy:".33em","text-anchor":"middle",fill:u.orange,stroke:u.base03})})}function e(){console.log("STARTING NEW GAME");for(var e;e=Snap.select(".gameOverText");)e.remove();Snap.select("#typeHistory").attr({text:""}),s=0,l=0,c=0,p=!1,M=[],T="",f=void 0,m=new Date,t(),a(),i()}function a(){b=25,n()}function n(){p||(b+=5,Snap.select("#wpmCounter").attr({text:"WPM: "+b}),setTimeout(n,15e3))}function i(){setTimeout(function(){s++;var t=Math.floor(w.length*Math.random()),e=Math.floor(v*Math.random()),a=w[t];if(M.push(a),!p){{S.group().attr({id:a}).text((1+e)*h/(v+1),x/16,a).attr({id:"TEXT"+a,fill:u.base1,"font-size":"25px","text-anchor":"middle"}).animate({y:14*x/16},5e3,mina.easeout,function(){var t=Snap.select("#"+a);if(t&&r(this.attr("x"),this.attr("y"),1),t&&(t.remove(),M=_.without(M,a),!p)){var n=Snap.select("#base"+e);n&&(n.remove(),g[e].alive=!1);var i=_.find(g,function(t){return t.alive});if(!i){p=!0,y=!0,setTimeout(function(){y=!1},5e3),d=new Date,m>d&&d.setDate(d.getDate()+1);var o=d-m;S.text(h/2,5*x/4,"GAME OVER").attr({"class":"gameOverText",fill:u.red,"font-size":"60px","text-anchor":"middle",stroke:u.base03,"stroke-width":1}).animate({y:2*x/8},1e3,mina.bounce),S.text(h/2,6*x/4,"Destroyed "+l+" of "+s+" words in "+Math.floor(o/1e3)+" seconds!").attr({"class":"gameOverText",fill:u.cyan,"font-size":"30px","text-anchor":"middle"}).animate({y:3*x/8},1e3,mina.bounce),S.text(h/2,7*x/4,"Actual words per minute: "+Math.floor(60*l*1e3/o)).attr({"class":"gameOverText",fill:u.cyan,"font-size":"30px","text-anchor":"middle"}).animate({y:4*x/8},1e3,mina.bounce),S.text(h/2,8*x/4,"Total keystroke mistakes: "+c).attr({"class":"gameOverText",fill:u.cyan,"font-size":"30px","text-anchor":"middle"}).animate({y:5*x/8},1e3,mina.bounce),setTimeout(function(){S.text(h/2,5*x/4,"Press any key to play again.").attr({"class":"gameOverText",fill:u.red,"font-size":"20px","text-anchor":"middle"}).animate({y:7*x/8},1e3,mina.bounce)},5e3)}}})}i()}},o())}function o(){return 6e4/b*(1+.5*(Math.random()-.5))}function r(t,e,a){a=0|a;var n=u.violet,i=u.red;1===a&&(n=u.red,i=u.violet),S.path(k).attr({fill:n,transform:"translate("+t+","+e+") scale(0.25)",stroke:i,"stroke-width":3}).animate({transform:"translate("+t+","+e+") scale(1.0)"},125,mina.bounce,function(){this.animate({transform:"translate("+t+","+e+") scale(0.25)"},50,mina.linear,function(){this.remove()})})}var s,l,c,f,m,d,u={base03:"#002b36",base02:"#073642",base01:"#586e75",base00:"#657b83",base0:"#839496",base1:"#93a1a1",base2:"#eee8d5",base3:"#fdf6e3",yellow:"#b58900",orange:"#cb4b16",red:"#dc322f",magenta:"#d33682",violet:"#6c71c4",blue:"#268bd2",cyan:"#2aa198",green:"#859900"},h=600,x=400,p=!0,y=!1,v=8,b=666,g=[],w=[],M=[],T="",S=Snap("#mainSVG");S.attr({width:h,height:x}),S.rect(0,0,h,x).attr({fill:"l(0, 0, 1, 1)"+u.base02+"-"+u.base01}),Snap.load("svg/backgroundSky.svg",function(t){S.append(t),S.path("M0,300 Q50,320 100,300  Q150,280 200,300 Q250,320 300,300  Q350,280 400,300  Q450,320 500,300  Q550,280 600,300 L600,400 L0,400").attr({fill:u.base2}),S.text(h/2,31*x/32,"").attr({id:"typeHistory",fill:u.base01,"font-size":"23px","text-anchor":"middle"}),S.text(h/64,63*x/64,"WPM: "+b).attr({id:"wpmCounter",fill:u.yellow,"font-size":"21px"}),$.get("txt/wordlist.txt",function(t){w=t.split(" "),S.text(h/2,5*x/4,"Type words to defend the city. Press any key to start.").attr({"class":"gameOverText",fill:u.red,"font-size":"20px","text-anchor":"middle"}).animate({y:7*x/8},1e3,mina.bounce)})}),$(document).bind("keyup",function(t){if(p)y||e();else{var a=String.fromCharCode(t.which).toLowerCase();T+=a;for(var n=!1,i=!1,o=0;o<M.length&&!n&&!i;o++){var s=M[o];s==T?n=!0:T==s.substring(0,T.length)&&(i=!0)}if(n){l++,S.text(h/2,31*x/32,T).attr({id:"typeHistory",fill:u.base01,"font-size":"23px","text-anchor":"middle"}).animate({y:17/16*x,"font-size":"1px"},500,mina.easeout,function(){this.remove()}),M=_.without(M,T);var f=Snap.select("#TEXT"+T);r(f.attr("x"),f.attr("y")),Snap.select("#"+T).stop().remove()}i||(T="",c+=1),Snap.select("#typeHistory").attr({text:T})}});var k;twoPi=6.28,k="M"+25*Math.sin(0)+","+25*Math.cos(0);for(var z=1;16>z;z++){var O;O=z%2===0?10*Math.random()+25:10*Math.random()+12,k+=" L"+O*Math.sin(twoPi*z/15)+","+O*Math.cos(twoPi*z/15)}k+=" L"+25*Math.sin(0)+","+25*Math.cos(0)}();